...
2. Tìm bucket lưu trữ media của bạn (ví dụ: `cloudforge-media-storage-...`).
...
2. Chọn ứng dụng frontend của bạn và nhấn **Actions > Delete app**.
3. Truy cập **CloudFront Console**.
4. Chọn distribution của bạn, nhấn **Disable**, và chờ trạng thái chuyển sang *Disabled* (có thể mất vài phút).
...
2. Vào phần **Clusters** và chọn cluster của bạn (ví dụ: `cloudforge-compute-cluster`).
3. Tại tab **Services**, chọn service backend của bạn, nhấn **Delete**, và xác nhận bằng cách gõ `delete`.
4. Khi service đã bị xóa, nhấn **Delete cluster** ở góc trên cùng bên phải.
5. Truy cập **Amazon ECR Console**.
6. Chọn repository của bạn (ví dụ: `cloudforge-backend`) và nhấn **Delete**.

#### Bước 4: Xóa Database (RDS & ElastiCache)
1. Truy cập **Amazon RDS Console**.
2. Chọn database instance của bạn (ví dụ: `cloudforge-db`).
...
6. Vào phần **Redis clusters**, chọn cluster của bạn và nhấn **Delete**.

#### Bước 5: Xóa Bảo mật và Mạng (WAF & VPC)
1. Truy cập **AWS WAF Console**.
2. Vào **Web ACLs** (đảm bảo bạn đang ở region *Global (CloudFront)*).
3. Chọn Web ACL của bạn, gỡ liên kết (disassociate) khỏi bất kỳ tài nguyên nào còn sót lại và nhấn **Delete**.
4. Truy cập **VPC Console**.
5. Vào phần **NAT Gateways**. Chọn NAT Gateway của bạn và nhấn **Delete**. *Bạn BẮT BUỘC phải đợi trạng thái chuyển thành 'Deleted' (khoảng 3-5 phút) trước khi làm bước tiếp theo.*
6. Vào phần **Elastic IPs**, chọn IP đã cấp phát, nhấn **Actions > Release Elastic IP address**.
7. Vào phần **Your VPCs**, chọn VPC của bạn, nhấn **Actions > Delete VPC**. Hành động này sẽ tự động dọn dẹp các subnet, route table và internet gateway liên quan.
